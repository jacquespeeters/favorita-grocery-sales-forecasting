...
for(delta in 1:16){
  ...
  metrics_tmp  = tibble(delta = delta, best_iteration = model_lgb$best_iter, best_score = model_lgb$best_score)
...
metrics %>% 
  write_csv(paste0("output/metrics", Sys.time(), ".csv"))

df_valid_pred %>% 
  write_csv("output/lgb_valid_pred.csv")
